...
<script type="text/javascript" src="who-is-online/jquery.min.js"></script>
<?php
//session_start();
$_SESSION['user_id'];
...
if($_SESSION['uid'])
{
$select=$DatabaseCo->dbLink->query("select * from register where index_id='$chat_user'");
...
?>
<?php
$select1=$DatabaseCo->dbLink->query("select * from payments where pmatri_id='$chat_user'");
		...
		$.each(data.items, function(i,item){
			...
			if (item)	{ // fix strange ie bug
				...
				//alert('user');
					...
					$("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage"><span class="chat_image"><img src="my_photos/'+item.ph+'" height=20px width=26px>&nbsp;</span><span class="chatboxmessagefrom">'+item.u+':&nbsp;&nbsp;</span><span class="chatboxmessagecontent">'+item.m+'</span></div>');
...
//alert(username);
	...
	$.ajax({
	  ...
	  url: "chat/chat?action=chatname&usw="+item.f,
