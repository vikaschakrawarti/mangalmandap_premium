         	...
         	<div class="content-wrapper">
            	...
            	<section class="content">
					<div class="row">
							...
							<div class="box-top updateSite">
								<div class="row">
									<div class="col-lg-3 col-sm-4">
										...
										<i class="fa fa-plus"></i>Add Star
										</a>
									</div>
									<div class="col-lg-3 col-xs-12 col-sm-4">
										<a href="updateWebSiteStar?star_status=all" class="btn btn-success btn-lg col-xs-12">
										<i class="fa fa-list"></i>All Star <span class="badge"><?php echo getRowCount("SELECT count(star_id) FROM star",$DatabaseCo);?></span>
										</a>
									</div>
									<div class="col-lg-3 col-xs-12 col-sm-4">
										<a href="updateWebSiteStar?star_status=approved" class="btn btn-success btn-lg col-xs-12">
										<i class="fa fa-thumbs-up"></i>Approved Star <span class="badge"><?php echo getRowCount("SELECT count(star_id) FROM star WHERE status='APPROVED'",$DatabaseCo);?></span>
										</a>
									</div>
									<div class="col-lg-3 col-xs-12 col-sm-4">
										<a href="updateWebSiteStar?star_status=unapproved" class="btn btn-success btn-lg col-xs-12">
										<i class="fa fa-thumbs-down"></i>Unapproved Star <span class="badge"><?php echo getRowCount("SELECT count(star_id) FROM star WHERE status='UNAPPROVED'",$DatabaseCo);?></span>
										</a>
									</div>
								</div>
							</div>
						...
						<?php
							$main_menu_count = getRowCount("SELECT count(star_id) FROM star".getWhereClauseForStatus($star_status),$DatabaseCo);
							if($main_menu_count>0){  
								$SQL_STATEMENT =  "SELECT * FROM star ".getWhereClauseForStatus($star_status)." ORDER BY star_id DESC";
						?>
						...
						<div class="col-xs-12 mt-10">
							<div class="box box-success">
								<div class="box-header">
									<h4><?php echo strtoupper($star_status); ?> STAR LIST</h4>
								</div>
								<!-- /.box-header -->
								<div class="box-body">
									<form method="post" action="updateWebSiteStar" id="action_form">
										<table id="example1" class="table table-bordered table-striped">
											...
											<tbody>
												...
												<tr>
													<td>
														<input type="checkbox" name="star_id[]" id="Item <?php  echo $DatabaseCo->dbRow->star_id;?>" class="second" value="<?php  echo $DatabaseCo->dbRow->star_id;?>"/>
														<label for="Item <?php  echo $DatabaseCo->dbRow->star_id;?>" class="label2">&nbsp;</label>	
													</td>
													<td>
														<a class="btn btn-default btn-sm md-trigger edit-popup"  href="javascript:;" data-modal="modal-13" data-id="<?php  echo $DatabaseCo->dbRow->star_id;?>" data-star="<?php  echo $DatabaseCo->dbRow->star;?>" data-star_status="<?php  echo $DatabaseCo->dbRow->status;?>"><i class="fa fa-pen fa-fw"></i><span class="hidden-xs">&nbsp;&nbsp;Edit</span>
														</a>
													</td>
													<?php
														$likeDisLikeCss = "";
													...
													<td class="updateSite"><span class="textUpdate"><?php  echo $DatabaseCo->dbRow->star;?></span></td>
												</tr>
												<?php } ?>
											</tbody>
										</table>
    	...
    	<div class="md-modal" id="modal-13">
			<div class="md-content" id="dialog">
				<div class="modal-header" >
					...
					<h4 class="modal-title" id="dialog_title">Add New Star</h4>
				</div>
				<div class='error-msg' id='validationSummary'></div>
				<form method="post" id="star-form" action="" method="post">
					<div class="modal-body gtSiteChangeId">
						<div class="form-group">
							<label for="exampleInputEmail1">Star</label>
							<input type="text" name="star" class="form-control" id="star_name" placeholder="Enter Star name">
						</div>
						<div class="form-group">
							<label>Status</label>
							<div class="radio">
								<input id="optionsRadios1" class="star_status" type="radio" checked="" value="APPROVED" name="star_status">
								<label for="optionsRadios1" class="mr-10">APPROVED</label>
								<input id="optionsRadios2" class="star_status" type="radio" value="UNAPPROVED" name="star_status">
					...
					<div class="modal-footer updateSite">
						...
						<input type="hidden" name="star_id" id="star_id" value=""/>
		...
		<!-- page script -->
      	<script type="text/javascript">
        	$(function() {
				...
				$("input[name=star_id]").click(function() {
				...
				$(document).on("click", ".edit-popup", function() {
					...
					var star_status = $(this).data('star_status');
					$("#star_id").val(myid);
					$("#star_name").val(star);
					$("#save").val("Update");
				...
				$("#save").button().click(function() {
					...
					var dataString = $("#star-form").serialize();
					$.ajax({
						type: "post",
						url: "web-services/add-details/add_star",
						dataType: "json",
						data: dataString,
						success: function(data) {
							if(data.successStatus) {
								...
								$("#validationSummary").show();
								$("#old").remove();
								$('<a href="updateWebSiteStar" id="old" class="md-close close">&times;</a>').appendTo('#new_button');
							} else {
