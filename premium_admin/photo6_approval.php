<?php
    ...
    if($isPostBack){     
        ...
        if(isset($_POST['index_id'])){
            ...
            switch($ACTION){
                case 'DELETE':		
                $SQL_STATEMENT =  "UPDATE register SET photo6='',photo6_approve='UNAPPROVED' WHERE index_id IN ".$index_val;
                $exe=$DatabaseCo->dbLink->query("SELECT photo6 from register WHERE index_id IN ".$index_val);
                while($get=mysqli_fetch_object($exe)){	
                    if($get->photo6!=''){	
                        unlink("../my_photos/".$get->photo6);
                        unlink("../my_photos_big/".$get->photo6);
                    }
                }
                break;
                case 'APPROVED':
...
<html>
	<head>
		<meta charset="UTF-8">
    	<title>Admin | Photo 6 Approval </title>
  	...
  	<body class="skin-blue">
      		...
      		<div class="content-wrapper">
				<section class="content-header">
					...
					<ol class="breadcrumb">
						...
						<li class="active">Photo 6 Approval</li>
					</ol>
				</section>
				<section class="content">
					<div class="row">
							...
							<div class="box-top updateSite">
								<div class="row">
									<div class="col-lg-3 col-xs-12 col-sm-4">
										<a href="photo6_approval.php?photo_status=all" class="btn btn-success btn-lg btn-block">
											<i class="fa fa-list"></i>All Photo 6 <span class="badge">
											<?php echo getRowCount("SELECT count(index_id) FROM register WHERE photo6!=''",$DatabaseCo);?></span>
										</a>
									</div>
									<div class="col-lg-3 col-xs-12 col-sm-4">
										<a href="photo6_approval.php?photo_status=approved" class="btn btn-success btn-lg btn-block">
											<i class="fa fa-thumbs-up"></i>Approved Photo 6 <span class="badge">
											<?php echo getRowCount("SELECT count(index_id) FROM register WHERE photo6!='' AND photo6_approve='APPROVED'",$DatabaseCo);?></span>
										</a>
									</div>
									<div class="col-lg-3 col-xs-12 col-sm-4">
										<a href="photo6_approval.php?photo_status=unapproved" class="btn btn-success btn-lg btn-block">
											<i class="fa fa-thumbs-down"></i>Unapproved Photo 6  <span class="badge">
											<?php echo getRowCount("SELECT count(index_id) FROM register WHERE photo6 !='' AND photo6_approve='UNAPPROVED'",$DatabaseCo);?></span>
										</a>
									</div>
									<div class="col-lg-3 col-xs-12 col-sm-4">
										<a href="photo6_approval.php?photo_status=pending" class="btn btn-success btn-lg btn-block">
											<i class="fa fa-clock-o"></i>Pending Photo 6 <span class="badge">
											<?php echo getRowCount("SELECT count(index_id) FROM register WHERE photo6 !='' AND photo6_approve='PENDING'",$DatabaseCo);?></span>
										</a>
									</div>
								</div>
							</div>
						...
						<?php
							$photo_count = getRowCount("SELECT count(index_id) FROM register".getWhereClauseForPhoto6($photo_status),$DatabaseCo);
							if($photo_count>0){  
								$SQL_STATEMENT =  "SELECT * FROM register".getWhereClauseForPhoto6($photo_status)." ORDER BY index_id DESC";
						?>
						...
						<div class="col-xs-12 mt-10">
							<div class="box box-success">
								...
								<!-- /.box-header -->
								<div class="box-body">
									<form method="post" action="photo6_approval" id="action_form">
											...
											<tbody>
												...
												<tr>
													...
													<?php
														...
														}elseif($DatabaseCo->dbRow->photo6_approve=="PENDING"){
													...
													<td title="<?php echo $DatabaseCo->dbRow->username;?>">
														<img src="../my_photos/watermark.php?image=<?php echo $DatabaseCo->dbRow->photo6; ?>&watermark=watermark.png" height="150px" width="130px" style="vertical-align:middle !important;cursor:pointer;" data-target="#myModal6" data-toggle="modal" onclick="Getphoto('<?php echo  $DatabaseCo->dbRow->matri_id;?>','6');"/>
													</td>
												</tr>
												<?php
													$rowCount++;
		...
		<div class="modal fade-in" id="myModal6" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"></div>
			...
			<script type="text/javascript">
				 setPageContext("approval","photo6-approve");
			</script>
