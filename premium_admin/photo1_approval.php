    ...
    if($isPostBack){     
        ...
        if(isset($_POST['index_id'])){
            ...
            switch($ACTION){
                ...
                $exe=$DatabaseCo->dbLink->query("SELECT photo1 FROM register WHERE index_id IN ".$index_val);
                while($get=mysqli_fetch_object($exe)){	
                    if($get->photo1!=''){	
                        unlink("../my_photos/".$get->photo1);
                        unlink("../my_photos_big/".$get->photo1);
                    }
                }
                break;
                case 'APPROVED':
...
<html>
	<head>
		<meta charset="UTF-8">
    	<title>Admin | Photo1 Approval </title>
        ...
        <!-- /. Icon Loader-->
      		...
      		<div class="content-wrapper">
        		<section class="content-header">
          			...
          			<ol class="breadcrumb">
            			...
            			<li class="active">Photo 1 Approval</li>
          			</ol>
        		</section>
				<section class="content">
					<div class="row">
							...
							<div class="box-top updateSite">
								<div class="row">
									<div class="col-lg-3 col-xs-12 col-sm-4">
										<a href="photo1_approval.php?photo_status=all" class="btn btn-success btn-lg btn-block">
											<i class="fa fa-list"></i>All Photo 1 <span class="badge">
											<?php echo getRowCount("SELECT count(index_id) FROM register WHERE photo1!=''",$DatabaseCo);?>
										</a>
									</div>
									<div class="col-lg-3 col-xs-12 col-sm-4">
										<a href="photo1_approval.php?photo_status=approved" class="btn btn-success btn-lg btn-block">
											<i class="fa fa-thumbs-up"></i>Approved Photo 1 <span class="badge">
											<?php echo getRowCount("SELECT count(index_id) FROM register WHERE photo1!='' AND photo1_approve='APPROVED'",$DatabaseCo);?></span>
										</a>
									</div>
									<div class="col-lg-3 col-xs-12 col-sm-4">
										<a href="photo1_approval.php?photo_status=unapproved" class="btn btn-success btn-lg btn-block">
											<i class="fa fa-thumbs-down"></i>Unapproved Photo 1 <span class="badge">
											<?php echo getRowCount("SELECT count(index_id) FROM register WHERE photo1 !='' AND photo1_approve='UNAPPROVED'",$DatabaseCo);?></span>
										</a>
									</div>
									<div class="col-lg-3 col-xs-12 col-sm-4">
										<a href="photo1_approval.php?photo_status=pending" class="btn btn-success btn-lg btn-block">
											<i class="fa fa-clock-o"></i>Pending Photo 1 <span class="badge">
											<?php echo getRowCount("SELECT count(index_id) FROM register WHERE photo1 !='' AND photo1_approve='PENDING'",$DatabaseCo);?></span>
										</a>
									</div>
								</div>
							</div>
						...
						<?php
							$photo_count = getRowCount("SELECT count(index_id) FROM register".getWhereClauseForPhoto1($photo_status),$DatabaseCo);
							if($photo_count>0){  
								$SQL_STATEMENT =  "SELECT * FROM register".getWhereClauseForPhoto1($photo_status)." ORDER BY index_id DESC";
						?>
						...
						<div class="col-xs-12 mt-10">
							<div class="box box-success">
								<div class="box-header">
									<h4>
										<?php echo strtoupper($photo_status); ?>  PHOTO 1 LIST
									</h4>
								</div>
								<div class="box-body gtMemPlan">
									<form method="post" action="photo1_approval" id="action_form">
											...
											<tbody>
												...
												<tr>
													...
													<?php
												        ...
												        }elseif($DatabaseCo->dbRow->photo1_approve=="PENDING"){
													...
													<td class="updateSiteApprovalStatus">
														<i class="fas <?php echo $likeDisLikeCss;?>"></i>
													</td>
													<td>
													...
													<td title="<?php echo $DatabaseCo->dbRow->username;?>">
														<img src="../my_photos/watermark.php?image=<?php echo $DatabaseCo->dbRow->photo1; ?>&watermark=watermark.png" height="150px" width="130px" style="vertical-align:middle !important;cursor:pointer;" data-target="#myModal6" data-toggle="modal" onclick="Getphoto('<?php echo  $DatabaseCo->dbRow->matri_id;?>','1');" />
													</td>
												</tr>
												<?php
													$rowCount++;
	 	...
	 	<div class="modal fade-in" id="myModal6" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"></div>
			...
			<script type="text/javascript">
				 setPageContext("approval","photo1-approve");
			</script>
